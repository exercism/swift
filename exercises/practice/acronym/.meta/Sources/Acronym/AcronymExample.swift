...
extension String {
  ...
  fileprivate func substringWithRangeInt(_ intRange: CountableRange<Int>) -> String {
  ...
  fileprivate func substringWithRangeInt(_ start: Int, end: Int) -> String {
    let range = start..<end
...
struct Acronym {
  ...
  static func abbreviate(_ inString: String) -> String {
    ...
    return splitStringToArray(insertSpaceAtCamelcase(inString)).map({
      $0.uppercased().substringWithRangeInt(0, end: 1)
    }).joined(separator: "")
  }
