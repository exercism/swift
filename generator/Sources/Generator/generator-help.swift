...
class GeneratorHelp{
    ...
    func getRemoteFiles() throws {
        let url = URL(string: "https://raw.githubusercontent.com/exercism/problem-specifications/master/exercises/\(exercise)/canonical-data.json")!
    ...
    func toml() throws {
        var toml = ""
        do {
            toml = try NSString(contentsOfFile: "\(self.filePath)/.meta/tests.toml",
    ...
    func checkConfig() throws -> Bool {
        ...
        var fileInfo : String
        do {
            fileInfo = try NSString(contentsOfFile: "../config.json",
                encoding: String.Encoding.ascii.rawValue) as String
