...
## Optional Binding in While Loops

You can use optional binding (`while let`) to keep running a loop as long as an operation continues to produce a non-`nil` value.
Once `nil` is returned, the loop stops:

```swift
var numbers = [1, 2]
while let count = numbers.popLast() {
    print("\(count)...")
}
print("Liftoff!")
